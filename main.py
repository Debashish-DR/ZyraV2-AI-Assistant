...
def setup_database():
    try:
        # Use environment variable for Render, fallback to .env for local
        mongodb_uri = os.environ.get('MONGODB_URI') or MONGODB_URI
        ...
        print("🔄 Using file-based storage mode")
        return None, None, None

# Initialize database
client, db, users = setup_database()
...
def AppendToChatLog(role, content, email=None, message_id=None):
    # Try MongoDB first
    if users is not None and email:
        try:
            chat_entry = {
                "email": email,
                "role": role,
...
def signup():
    try:
        ...
        # Check if user already exists (file-based if no MongoDB)
        if users is not None:
            existing_user = users.find_one({'email': email})
            if existing_user:
                return jsonify({'error': 'User already exists with this email'}), 400
        else:
            # Simple file-based check for development
            chatlog_path = get_user_chatlog_path(email)
            if os.path.exists(chatlog_path):
                return jsonify({'error': 'User already exists with this email'}), 400
        ...
        # Create user (MongoDB or file-based)
        if users is not None:
            user_data = {
                'email': email,
...
def login():
    try:
        ...
        # Find user (MongoDB or file-based)
        user = None
        if users is not None:
            user = users.find_one({'email': email})
        else:
            # File-based user check - if chatlog exists, user exists
            chatlog_path = get_user_chatlog_path(email)
            if os.path.exists(chatlog_path):
                user = {'email': email, 'username': 'User', 'assistantname': 'Zyra', 'assistantvoice': 'en-CA-ClaraNeural'}
        
        if not user:
            return jsonify({'error': 'Invalid email or password'}), 401
        
        # Verify password (only for MongoDB users)
        if users is not None:
            try:
...
# Other routes remain similar but simplified...
    ...
    try:
        ...
        if users is not None:
            ...
            if user_data:
                ...
                assistantname = user_data.get('assistantname', 'Zyra')

        Decision = FirstLayerDMM(message)
        response = None

        if Decision and any(any(query.startswith(func) for func in ["open", "close", "play", "system", "call", "message"]) for query in Decision):
            # Handle automation commands
            if IS_RENDER:
                response = "🔒 This feature works on local Windows devices. Try: 'search python' or 'what is AI?'"
            else:
                results = []
                for query in Decision:
                    if any(query.startswith(func) for func in ["open", "close", "play", "system", "call", "message"]):
                        result = ExecuteCommand(query)
                        results.append(result)
                response = " ".join(results)
        else:
            # Handle AI chat
            if any(query.startswith("realtime") for query in Decision if isinstance(query, str)):
                response = RealtimeSearchEngine(message)
            else:
                response = ChatBot(message, username, assistantname)
