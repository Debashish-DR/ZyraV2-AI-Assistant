...
try:
    # For Render.com - use environment variable directly
    MONGODB_URI = os.environ.get('MONGODB_URI') or env_vars.get("MONGODB_URI")
    
    if not MONGODB_URI:
        print("⚠️ No MongoDB URI found - using file-based storage")
        client = None
        db = None
        users = None
    else:
    ...
    print("🔄 Using file-based storage mode (chat will still work)")
    client = None
    db = None
    users = None
